...
FORMAT_FLAG = ["p", "p", "p", "p", "p"]
FORMAT_SEP = ","
HEADER = ["ID", "Label", "Date", "Produced By", "Classified As"]
...
def get_filtered_objects(filters, DATABASE):
    ...
    try:
        ...
        uri=True) as connection:
            ...
            with closing(connection.cursor()) as cursor:
                ...
                stmt_str = f"""
                            select objects.id,  objects.label, temp1.date, group_concat(distinct (temp1.name || ' ('||temp1.part||')' )) as agents_name, 
                                 group_concat(distinct temp2.name ) as classifiers_name
